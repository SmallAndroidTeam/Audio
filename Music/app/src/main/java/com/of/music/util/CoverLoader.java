...
public class CoverLoader {
    ...
    public Bitmap loadThumb(Imusic music) {
        return loadCover(music, Type.THUMB);
    }

    public Bitmap loadRound(Imusic music) {
        return loadCover(music, Type.ROUND);
    }

    public Bitmap loadBlur(Imusic music) {
        return loadCover(music, Type.BLUR);
    }

    private Bitmap loadCover(Imusic music, Type type) {
        Bitmap bitmap;
    ...
    private String getKey(Imusic music) {
        if (music == null) {
            return null;
        }

        if (music.getType() == Imusic.Type.LOCAL && music.getAlbumId() > 0) {
            return String.valueOf(music.getAlbumId());
        } else if (music.getType() == Imusic.Type.ONLINE && !TextUtils.isEmpty(music.getCoverPath())) {
    ...
    private Bitmap loadCoverByType(Imusic music, Type type) {
        Bitmap bitmap;
        if (music.getType() == Imusic.Type.LOCAL) {
