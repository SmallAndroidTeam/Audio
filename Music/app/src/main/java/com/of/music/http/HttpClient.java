...
import com.of.music.model.Splash;
...
public class HttpClient {
    ...
    public synchronized  static void getMusicDownloadInfo(String songId, @NonNull final HttpCallback<DownloadInfo> callback) {
        Log.i("Url", "getMusicDownloadInfo: "+songId);
        
        OkHttpUtils.post().url(BASE_URL)
                ...
                .execute(new JsonCallback<DownloadInfo>(DownloadInfo.class) {
                    ...
                    public void onResponse(DownloadInfo response, int id) {
                        Log.i("Url", "onSuccess: "+response.getBitrate().getFile_link());
                        callback.onSuccess(response);
                    }

                    @Override
                    public void onError(Call call, Exception e, int id) {
                        Log.i("Url", "onResponse:2 ");
                        callback.onFail(e);
                    }

                    @Override
                    public void onAfter(int id) {
                        Log.i("Url", "onResponse:3 ");
                        callback.onFinish();
                    }
                });
    }
    ...
    public static void searchMusic(String keyword, @NonNull final HttpCallback<ISearchMusic> callback) {
        OkHttpUtils.get().url(BASE_URL)
                .addParams(PARAM_METHOD, METHOD_SEARCH_MUSIC)
                .addParams(PARAM_QUERY, keyword)
                .build()
                .execute(new JsonCallback<ISearchMusic>(ISearchMusic.class) {
                    @Override
                    public void onResponse(ISearchMusic response, int id) {
                        callback.onSuccess(response);
                    }
